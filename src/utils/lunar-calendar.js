 ...
 * @charset UTF-8
 * @Author yuteng inspiration source by Jea杨
 * @Time  2021-4-1
 * @Version 1.0.0
...
const calendar = {
   ...
   * @param m lunar month
   * @param d lunar day
   * @param flag lunar month is leap or not.[如果是农历闰月第四个参数赋值true即可]
   * @return JSON object
   * @eg:console.log(calendar.lunar2solar(1987,9,10));
   */
  lunar2solar: function(y, m, d, flag) {
    const isLeapMonth = !!isLeapMonth
...
export function solar2lunar(y, m, d) {
  ...
  // 返回当月「节」为几日开始
  const firstNode = calendar.getTerm(year, (m * 2 - 1))
  // 返回当月「节」为几日开始
  const secondNode = calendar.getTerm(year, (m * 2))
  // 依据12节气修正干支月
